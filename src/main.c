...
void mashing_task(void *pvParameters) {
    ...
    while (1) {
        ...
        uint32_t interval_ms = (cfg && cfg->control_interval_ms > 0) ? cfg->control_interval_ms : 5000;
            ...
            case STATE_HOLDING:
                {
                    ...
                    if (time_left_s <= 0) {
                        ...
                        if (current_temp < (target_temp - cfg->hysteresis_down)) {
                            ...
                            if (slope < -0.01f) target_stage = 2;
                            else if (slope > 0.01f) target_stage = 0;
                            else target_stage = 1;
                        }
                        
                        // Enforce hold cap max 3
                        if (target_stage > 3) target_stage = 3;
                    }
                }
                break;
        ...
        if (target_stage != last_stage) {
            ESP_LOGI(TAG, "Stage changed: %d -> %d (Current Temp: %.2f, Target: %.2f)", 
        ...
        mqtt_publish_status(current_temp, target_temp, last_stage, current_state, time_left_s);
